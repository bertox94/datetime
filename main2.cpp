...
public:
    standing_order(const string &name, bool is_wire_transfer,
                   const date &initialDate, bool ends,
                   int f1, const string &f2,
                   double amount) :
            name(name), is_wire_transfer(is_wire_transfer),
            initial_date(initialDate), ends(ends),
            f1(f1), f2(f2),
            amount(amount) {
        if (ends)
            throw std::runtime_error("Parameter \"end\" must be false!");
    }

    standing_order(const string &name, bool is_wire_transfer,
                   const date &initialDate, bool ends, const date &finalDate,
                   int f1, const string &f2,
                   double amount)
            : name(name), is_wire_transfer(is_wire_transfer),
              initial_date(initialDate), ends(ends), final_date(finalDate),
              f1(f1), f2(f2),
              amount(amount) {
        if (!ends)
            throw std::runtime_error("Parameter \"end\" must be true!");
    }
...
std::vector<standing_order> waiting_queue;

bool execute(date today, const standing_order &row) {
...
bool is_today(standing_order &row, date today) {
...
void process(ofstream &myfile, date &today, double &var, standing_order &el) {
